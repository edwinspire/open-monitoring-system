...
$result = $db->query_params_result_as_json("SELECT account, last_name, first_name, geox, geoy, address, address_ref FROM accounts WHERE enabled = true AND geox != 0 AND geoy != 0", array());
break;
case 1:
...
$result = $db->query_params_result_as_json("SELECT account, last_name, first_name, geox, geoy, address, address_ref FROM accounts WHERE idcontact = $1::bigint AND geox != 0 AND geoy != 0", array($_GET["idcontact"]));
}
break;
case 2:
...
$result = $db->query_params_result_as_json("SELECT account, last_name, first_name, geox, geoy, address, address_ref FROM accounts WHERE idcontact IN (SELECT idaccount FROM view_account_contacts WHERE idcontact = $1::bigint AND appointment = 'Técnico Responsable') AND geox != 0 AND geoy != 0;", array($_GET["idcontact"]));
