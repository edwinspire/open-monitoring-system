// Dojo 1.7+ (AMD)
"dojo/promise/all"
require(["dojo/_base/lang", "api/scheduled_tasks/scheduled_tasks",   "dojo/Deferred", "dojo/_base/array", "dojo/promise/all", "dojo/on", "dojo/node!mssql"], function(lang, Octopus, Deferred, array, all, on, mssql){
	lang.extend(Octopus, {
/////////////////////////////////////////
run_tareasETARM: function(task){
	
	var deferred = new Deferred();
	var t = this;
	console.log(task);

	mssql.connect({
		user: task.task_parameters.mssql.user,
		password: task.task_parameters.mssql.pwd,
    server: task.task_parameters.mssql.ip, // You can use 'localhost\\instance' to connect to named instance 
    database: 'msdb',
    requestTimeout: 300000,
    options: {
        encrypt: true // Use this if you're on Windows Azure 
    }
}).then(function(cnxmatriz) {
    // Query 
    var msSQLConnection = this;    

    var srtquery = `
SELECT DISTINCT cjs_nombre_job, cjs_id_oficina as oficina,  *
  FROM [bdgeneral].[log].[tbl_ControlTrabajoServidor] where cjs_fecha_ejecucion > DATEADD(day,-1, CURRENT_TIMESTAMP) order by cjs_fecha_ejecucion, cjs_id_oficina;
    `;

    new mssql.Request(cnxmatriz)
    .query(srtquery).then(function(recordset) {

    	//console.log(recordset);
    	var Filas = recordset.length;

    	if(Filas > 0){


    		var Message = `   		
    		<table style="border: 1px solid black; border-collapse: collapse;">
    		<tr>
    		<th style="border: 1px solid black; border-collapse: collapse;">OFICINA</th>
    		
    
    		<th style="border: 1px solid black; border-collapse: collapse;">TAREA</th>
    		
    		<th style="border: 1px solid black; border-collapse: collapse;">FECHA</th>
    		
    		<th style="border: 1px solid black; border-collapse: collapse;">ESTADO</th>
    		</tr>
    		`;


    		array.forEach(recordset, function(reg){
    			ofi = reg.cjs_id_oficina;
    			fecha = reg.cjs_fecha_ejecucion;
    			tarea = reg.cjs_nombre_job;
    			estado = reg.cjs_estado;

    			Message = Message+`
    			<tr>
    			<td style="border: 1px solid black; border-collapse: collapse;">${ofi}</td>
    			
    			<td style="border: 1px solid black; border-collapse: collapse;">${tarea}</td>
    			
    			<td style="border: 1px solid black; border-collapse: collapse;">${fecha}</td>
    			
    			<td style="border: 1px solid black; border-collapse: collapse;">${estado}</td>
    			</tr>
`;

    		});

    		Message = Message+`
    		</table>
    		`;

    		t.send_event_pg('events', {idaccount: task.task_parameters.idaccount, ideventtype: 140, description: Message}, []).then(function(result){
    			deferred.resolve(true);
    		});


    	}else{
    		deferred.resolve(false);
    	}

    }).catch(function(err) {

    	console.log(err);
    	deferred.resolve(false);
    });


}).catch(function(err) {

	console.log(err);
	deferred.resolve(false);
});

return deferred.promise;
}




});
});

