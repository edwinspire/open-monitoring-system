...
function parseArguments(testArgs) {
    const { nodeUnit, remoteUnit, remoteFunctional, childConfig, internConfig, reporters, testingKey, userName, filter } = testArgs;
...
function default_1(testArgs) {
    return __awaiter(this, void 0, void 0, function* () {
        const testRunPromise = new Promise((resolve, reject) => {
            ...
            if (testArgs.verbose) {
                logger(`${chalk_1.blue.bold('  Intern config:')}`);
                logger('    ' + chalk_1.blue(String(cs.sync(internPath, ['showConfig', ...internArgs]).stdout)));
                logger(`${chalk_1.blue.bold('  Parsed arguments for intern:')}`);
                logger('    ' + chalk_1.blue(String(internArgs.join('\n    '))));
            }
            cs
