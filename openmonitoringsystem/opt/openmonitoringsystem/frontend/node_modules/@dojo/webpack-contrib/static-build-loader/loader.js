...
function hasCheck(hasIdentifier, hasNamespaceIdentifier, args, callee) {
...
function existsCheck(existsIdentifier, hasNamespaceIdentifier, args, callee) {
    return ((namedTypes.Identifier.check(callee) && callee.name === existsIdentifier && args.length === 1) ||
        (namedTypes.MemberExpression.check(callee) &&
            namedTypes.Identifier.check(callee.object) &&
            callee.object.name === hasNamespaceIdentifier &&
            namedTypes.Identifier.check(callee.property) &&
...
function addCheck(addIdentifier, hasNamespaceIdentifier, args, callee) {
    return ((namedTypes.Identifier.check(callee) &&
...
function loader(content, sourceMap) {
    ...
    let hasIdentifier;
    let hasNamespaceIdentifier;
    let existsIdentifier;
    let addIdentifier;
    let comment;
    ...
    if (hasIdentifier || hasNamespaceIdentifier || existsIdentifier || addIdentifier) {
        types.visit(ast, {
            visitCallExpression(path) {
                ...
                const isHasCheck = hasCheck(hasIdentifier, hasNamespaceIdentifier, args, callee);
                const isExistsCheck = existsCheck(existsIdentifier, hasNamespaceIdentifier, args, callee);
                const isAdd = addCheck(addIdentifier, hasNamespaceIdentifier, args, callee);
                ...
                if (isAdd) {
                    const [feature] = args;
                    if (namedTypes.Literal.check(feature) &&
                        typeof feature.value === 'string' &&
                        feature.value in features) {
                        path.replace(builders.callExpression(callee, [args[0], builders.literal(features[feature.value])]));
                    }
                    return false;
                }
                this.traverse(path);
