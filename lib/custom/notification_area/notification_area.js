define(['dojo/_base/declare',
    ...
    'dojo/request',
'dojo/on'
], function (declare, _Widget, _Templated, templateString, domConstruct, domStyle, R, on) {
...
on.emit(window, "on_table_data_has_changed", tables);
...
//alert('El navegador No soporta EventSource');
	...
	window.notification_area = function(args_){
		t.notify(args_);
	}
        ...
        _args: function (a) {
            ...
            if (a.snd === undefined || a.snd.length < 5) {
                a.snd = 'notifications_media/snd/snd00.mp3';
            }
...
//console.log('Deberia mostrar una notify');
                                t.notify({message: item.body, title: item.title, img: item.img, snd: item.snd, timeout: item.timeout, urgency: item.urgency, closable: item.closable});
                                t._lastidnotify = ix;
                            }
                        });
