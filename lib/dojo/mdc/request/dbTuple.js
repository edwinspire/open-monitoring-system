define("mdc/request/dbTuple", ["dojo/_base/declare", 'dojo/request', 'dojo/query', "dojo/Evented"], function (_1, _2, _3, _4) {

/// TODO ///
// Agregar la posibilidad de agregar campos adicionales segun el id
...
//console.error('--->>> '+wi.attr("required"));
if(!t._isvalid && !wi.attr("required")){
	t._isvalid = true;
	t._data[name] = null;
console.log('--->>> El campo '+name+' Es invalido pero no es requerido, se lo setea a null');
}else{
        t._data[name] = t._value_field_to_pg(wi.get('data-dbfieldtype'), wi.get("value"));
}

if(wi.get('data-dbfieldemptyasnull') && t._data[name].length < 1){
 t._data[name] = null;
}
...
console.log(_query_type);
                            switch (_query_type) {
                                case 'load':
console.warn("Esta opcion ya esta decreptada");
                                    t._data = response[0];
