...
header('HTTP/1.1 401 Unauthorized', true, 401);
        ...
        switch ($_POST["query_type"]) {
            ...
            case "insert":
		$result = "{\"insert\": \"".$db->insert("contacts", $_POST, array("idcontact"))."\"}";
                break;

            case "update":
                if (isset($_POST["idcontact"])) {
		$result = "{\"updated\": \"".$db->update("contacts",  
							$_POST, 
            ...
            case "select":
                if (isset($_POST["idcontact"])) {
		$result = $db->select_result_as_json("contacts", array(), array("idcontact"=>$_POST["idcontact"]));
                }
                break;
            case "delete":
                if (isset($_POST["idcontact"])) {
		$result = "{\"deleted\": \"".$db->delete("contacts", array("idcontact"=>$_POST["idcontact"]))."\"}";
                }else{
