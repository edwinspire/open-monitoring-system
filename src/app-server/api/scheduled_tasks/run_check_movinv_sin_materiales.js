...
run_check_movinv_sin_materiales: function(task){
    ...
    .query(srtquery).then(function(recordset) {
     ...
     t.send_event_pg({idaccount: task.task_parameters.idaccount, ideventtype: ideventtype, description: '<b>Listado: </b>'+materiales.join(' '), details: JSON.stringify(recordset)}, []).then(function(result){
